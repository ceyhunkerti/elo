...
pub fn info(self: Source) anyerror![]const u8 {
    return self.vtable.info(self.ptr);
}

pub fn run(self: Source, wire: *w.Wire) anyerror!void {
    return self.vtable.run(self.ptr, wire);
}

pub fn prepare(self: Source, options: ?std.StringHashMap([]const u8)) anyerror!void {
