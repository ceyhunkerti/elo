...
insert_metadata: InsertMetadata = undefined,
...
test "batchInsert" {
    ...
    try conn.execute("create table test_table (idx number, name varchar2(255))");

    const column_count = 2;
    var dpi_var_array: []?*c.dpiVar = try allocator.alloc(?*c.dpiVar, column_count);
    var data_array: []?[*c]c.dpiData = try allocator.alloc(?[*c]c.dpiData, column_count);
    ...
    for (0..column_count) |i| {
        if (c.dpiStmt_bindByPos(stmt.stmt, @as(u32, @intCast(i)) + 1, dpi_var_array[i]) < 0) {
            unreachable;
        }
    }
    ...
    if (c.dpiStmt_executeMany(stmt.stmt, c.DPI_MODE_EXEC_DEFAULT, 2) < 0) {
        unreachable;
    }
